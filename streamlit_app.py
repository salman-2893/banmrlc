...
def compile_pdf_report(dataframe):
    ...
    # PDF Headers Layout (Removed Emojis to avoid rendering exceptions, updated layout args)
    ...
    for _, row in dataframe.iterrows():
        for col in pdf_cols:
            ...
            if val_str == "nan":
                val_str = ""
            # Clean text values of hidden emoji codes if any exist in the data strings
            val_cleaned = val_str.encode('ascii', 'ignore').decode('ascii')
            pdf.cell(col_width, 7, val_cleaned[:20], border=1)
        pdf.ln()
        
    # Standardize output bytes parsing to eliminate binary data format errors
    pdf_output = pdf.output()
    if isinstance(pdf_output, (bytearray, bytes)):
        return bytes(pdf_output)
    
    # Fallback structure buffer write 
    buffer = io.BytesIO()
...
# ------------------------------------------------------------------------------
# TAB 1: DASHBOARD AND PDF DOWNLOAD
# ------------------------------------------------------------------------------
with tab_dashboard:
