...
def fetch_live_data():
    try:
        ...
        st.error(f"Could not load data. Ensure your Sheet is shared as 'Anyone with the link can view'. Error: {e}")
...
# 4. COMPUTE PDF LOGIC (Using Free FPDF2)
def compile_pdf_report(dataframe):
    ...
    # Title
    pdf.cell(0, 10, "🎖️ Contingent Deployment & Operational Query Report", ln=True)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 6, f"Total Rows Matching Parameters: {len(dataframe)} Record(s)", ln=True)
    pdf.ln(5)
    ...
    col_width = 34  # Balanced fits for A4 Landscape bounds
    for col in pdf_cols:
    ...
    for _, row in dataframe.iterrows():
        for col in pdf_cols:
            ...
            if val_str.endswith('.0') and col in ['no', 'phone']:
                val_str = val_str[:-2]
            pdf.cell(col_width, 7, val_str[:20], border=1) # Truncate gently if field clips bounds
        pdf.ln()
...
with tab_dashboard:
    ...
    if not filtered_df.empty:
        try:
            pdf_bytes = compile_pdf_report(filtered_df)
            st.download_button(
            ...
            st.warning(f"PDF Generator initialized. Ready for print layout commands.")
